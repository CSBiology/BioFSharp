...
For this there are two types of amino acids defined: `AminoAcidSymbol` is a very primitive type basically only consisting of a one letter code. It is located in the `AminoAcidSymbols` module. This type should be used for efficient pattern matching. On the other hand there is the `AminoAcid` type. This type uses the `IBioItem` interface and is packed with functionality. It's located in the 'AminoAcids' module For more complex applications this type should be used.
...
In principle you have two different ways of defining values of type `AminoAcid`. You can either directly call the union case of the type:
...
Most of the functinality the `AminoAcid` type offers comes from the `IBioItem` interface. As in F#, you have to explicitly implement interfaces in classes, accessing those functionalities can be tricky at first. More on this [here](https://fsharpforfunandprofit.com/posts/interfaces/). 
...
(** 
Many built-in functions of the `BioItem` can be found in the `AminoAcids` module. Those already do the casting for you.
Therefore the manual casting is only necessary if you want to use functionalities of the IBioItem in special, self created functions (Or if you really really prefer using members over functions).  
To make this a bit clearer:
The variable myGlycine is now of type `IBioItem`. The following three statements will print the same phrase:
